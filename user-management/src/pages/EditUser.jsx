...
export default function EditUser() {
  ...
  async function handleSubmit(e) {
    ...
    const isLocalUser = numId > 100; // naye users ka id Date.now() hota hai (13 digits)

    try {
      if (isLocalUser) {
        // Naya user hai — API call mat karo, seedha context update karo
        updateUser({ id: numId, ...payload });
        toast(
        ...
        updateUser({ id: numId, ...updated });
        toast(
          `${updated.firstName} ${updated.lastName} updated successfully!`,
          "success",
        );
        navigate("/");
      }
    ...
    <>
      ...
      <div className="page-header">
        <div className="page-header-left">
          <h1>Edit User</h1>
          <p>
            Update the information for <strong>{originalName}</strong> (ID: {id}
            )
          </p>
        </div>
