...
export default function Users() {
  ...
  // ── Delete flow ────────────────────────────
  function askDelete(user) { setDeleteTarget(user) }

  async function confirmDelete() {
    if (!deleteTarget) return
    setDeleting(true)
    try {
      const res = await fetch(`https://dummyjson.com/users/${deleteTarget.id}`, {
        method: 'DELETE'
      })
      if (!res.ok) throw new Error(`HTTP ${res.status}`)
      // Update the SHARED list — no re-fetch, no blank screen
      removeUser(deleteTarget.id)
      toast(`${deleteTarget.firstName} ${deleteTarget.lastName} deleted`, 'success')
    } catch (err) {
